...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('medical_record_loan', function (Blueprint $table) {
            ...
            $table->bigInteger('patient_id')->nullable()->index()->on('patient')->constrained()->cascadeOnDelete();
            $table->integer('division_unit_id', false)->nullable()->index()->comment('ID divisi/unit/instalasi dari si peminjam')->on('division_unit')->constrained()->cascadeOnDelete();
            ...
            $table->bigInteger('created_by')->nullable()->index()->comment('id dari user yang menginput, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->timestamp('created_at')->nullable();
            $table->bigInteger('updated_by')->nullable()->index()->comment('id dari user yang mengupdate, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->timestamp('updated_at')->nullable();
            $table->bigInteger('delete_by')->nullable()->index()->comment('id dari user yang menghapus, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->softDeletes();
        });

        Schema::table('medical_record_loan', function (Blueprint $table) {
            $table->foreign('loan_reference_id')->references(['id'])->on('medical_record_loan')->constrained()->cascadeOnDelete();
        });
    }
