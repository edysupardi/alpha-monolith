...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('medical_record_loan', function (Blueprint $table) {
            ...
            $table->integer('company_id', false)->index()->comment('ID dari perusahaan/PT/CV')->references('id')->on('company')->constrained()->cascadeOnDelete();
            $table->integer('branch_id', false)->index()->comment('ID cabang dari si perusahaan')->references('id')->on('branch')->constrained()->cascadeOnDelete();
            $table->bigInteger('patient_id')->index();
            $table->integer('division_unit_id', false)->index()->comment('ID divisi/unit/instalasi dari si peminjam')->references('id')->on('division_unit')->constrained()->cascadeOnDelete();
            ...
            $table->bigInteger('created_by')->index()->comment('id dari user yang menginput');
