...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('examination', function (Blueprint $table) {
            ...
            $table->integer('branch_id', false)->nullable()->comment('ID cabang dari si perusahaan')->on('branch')->constrained()->cascadeOnDelete();
            $table->bigInteger('patient_id', false)->nullable()->index()->comment('ID dari orang nya')->on('patient')->constrained()->cascadeOnDelete();
            $table->dateTime('examination_date')->nullable()->comment('tanggal perawatannya');
            $table->bigInteger('doctor_id', false)->nullable()->index()->comment('id dokter nya')->on('employee')->constrained()->cascadeOnDelete();
            ...
            $table->bigInteger('created_by', false)->nullable()->index()->comment('id dari user yang menginput, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->dateTime('created_at')->nullable()->useCurrent();
            $table->bigInteger('updated_by', false)->nullable()->index()->comment('id dari user yang mengupdate, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->dateTime('updated_at')->nullable()->useCurrentOnUpdate()->useCurrent();
            $table->bigInteger('delete_by', false)->nullable()->index()->comment('id dari user yang menghapus, walaupun sama antara dokter dan user')->on('employee')->constrained()->cascadeOnDelete();
            $table->softDeletes();
        });
    }
